<?php
	...
	$sql = "SELECT * FROM orders WHERE order_date = :value1 
			AND status = :value2 
			ORDER BY id DESC";
									...
									<tbody>
										<?php 
											...
											foreach ($pending_orders as $pending_order) {
												...
												$pending_vouche_no = $pending_order['vouche_no'];
												$pending_total = $pending_order['total'];
											
										?>
										<tr>
											<td> <?= $i++; ?> </td>
											<td> <?= $pending_vouche_no; ?> </td>
											<td> <?= $pending_total; ?> </td>
											<td>
												<a href="invoice.php" class="btn btn-outline-info">
													<i class="icon icofont-info"></i>
												</a>

												<a href="orderstatus_change.php?id=<?= $pending_id ?>&status=0" class="btn btn-outline-primary">
													<i class="icon icofont-ui-check"></i>
												</a>
									...
									<tbody>
										<?php 
											...
											foreach ($confirm_orders as $confirm_order) {
												...
												$confirm_vouche_no = $confirm_order['vouche_no'];
												$confirm_total = $confirm_order['total'];
											
										?>
										<tr>
											<td> <?= $i++; ?> </td>
											<td> <?= $confirm_vouche_no; ?> </td>
											<td> <?= $confirm_total; ?> </td>
											<td>
												<a href="" class="btn btn-outline-primary">
													<i class="icon icofont-info"></i>
									...
									<tbody>
										<?php 
											...
											foreach ($cancel_orders as $cancel_order) {
												...
												$cancel_vouche_no = $cancel_order['vouche_no'];
												$cancel_total = $cancel_order['total'];
											
										?>
										<tr>
											<td> <?= $i++; ?> </td>
											<td> <?= $cancel_vouche_no; ?> </td>
											<td> <?= $cancel_total; ?> </td>
											<td>
												<a href="" class="btn btn-outline-danger">
													<i class="icon icofont-info"></i>
